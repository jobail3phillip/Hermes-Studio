...
// Explicit assignment/ownership/execution/review markers actually present in
// the live corpus. 'Confirmed by' is deliberately excluded: it is an intake
// CONFIRMATION signal (JP confirms nearly every intake record by governance
// design), not evidence of who owns/executes/reviews the work — using it for
// resource attribution collapses the distribution onto JP. Only fields that
// genuinely indicate assignment, execution, or review are used; anything
// else falls back to 'Unassigned' rather than guessing.
const ASSIGNMENT_FIELD_RE = /\*{0,2}(Executed by|Execution Authority(?:\s*\/\s*Mission Owner)?|Mission Owner|Assigned to|Routed to|Owner|Review Authority|Reviewed by)\*{0,2}\s*:\s*\*{0,2}\s*([^\n]+)/gi
...
// No guessing tier: resource comes only from a genuine ownership/assignment/
// execution/review field match. If none is present, the record is
// 'Unassigned' — never inferred from a bare first-actor-anywhere text scan.
function extractResource(text: string): string {
  const clean = text.replace(GENERATOR_FOOTER_RE, '')
  for (const m of clean.matchAll(ASSIGNMENT_FIELD_RE)) {
